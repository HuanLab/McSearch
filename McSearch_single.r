#########################################
# This script is to do the single McSearch against MCS library.
# Shipei Xing, Mar 12, 2020
# Copyright @ The University of British Columbia

#########################################
# Parameter setting part
data.path <-'E:/McSearch/R_script/single_search'        # Data path for "single_search" folder. Use "/" instead of "\".
db.name <- 'Fiehn HILIC_MCS library.msp'                # The MS/MS spectral database name ending with ".msp"
mz.tol <- 0.01                                          # The m/z tolerance used for MCS library construction (default 0.01 Da)
topNo <- 100                                            # The number of highest HNL peaks used for spectral matching, default 100.
...
adduct.No <- 1                                          # Index of adduct type: 
                                                        #    Positive: 1.[M+H]+ 2.[M]+ 3.[M+NH4]+ 4.[M+Na]+ 5.[M+CH3OH+H]+ 6.[M+K]+ 7.[M+Li]+ 8.[M+ACN+H]+ 9.[M+H-H2O]+ 10.[M+H-2H2O]+ 
                                                        #              11.[M+2Na-H]+ 12.[M+Isoprop+H]+ 13.[M+ACN+Na]+ 14.[M+2K-H]+ 15.[M+DMSO+H]+ 16.[M+2ACN+H]+ 17.[M+IsoProp+Na+H]+
                                                        #              18.[M-C6H10O4+H]+ 19.[M-C6H10O5+H]+ 20.[M-C6H8O6+H]+
                                                        #    Negative: 1.[M-H]- 2.[M]- 3.[M-H2O-H]- 4.[M+Na-2H]- 5.[M+Cl]- 6.[M+K-2H]- 7.[M+FA-H]- 8.[M+Hac-H]- 9.[M+C2H3N+Na-2H]- 
                                                        #              10.[M+Br]- 11.[M+TFA-H]- 12.[M-C6H10O4-H]- 13.[M-C6H10O5-H]- 14.[M-C6H8O6-H]- 15.[M+CH3COONa-H]-
premass.Q <- 309.1810                                   # The precursor m/z of the query spectrum
mass.diff <- 200                                        # Searching mass range, default 200 Da. 
                                                        # Reference spectra with precursor m/z difference smaller than mass.diff will be considered in the following search.
...
# input: csv file
# output: csv file
# directed CSS score (library compounds as templates)
...
setwd(data.path)
#input csv
ms2.Q <- read.csv('input.csv',stringsAsFactors = FALSE)
setwd(paste0(data.path,'/files'))
#input smiles.db, msp library
if(db.name=='MoNA_MCSlibrary.msp'){smiles.db <- read.csv('smiles_db.csv',stringsAsFactors = FALSE)}
database <- read.msp(db.name, only.org = FALSE,
                     org.set = c('C','H','N','O','P','S','F','Cl','Br','I'), noNumbers = NULL)
adduct.table <- read.csv('adduct info.csv',stringsAsFactors = FALSE)
...
colnames(element.table) <- c('element','number','mass')
element.table[,1] <- c('C','H','N','O','P','S','F','Cl','Br','I')
element.table[,2] <- rep(0,10)
element.table[,3] <- c(12,1.007825,14.003074,15.994915,30.973762,31.972071,18.998403,34.968853,78.918337,126.904473)
...
# premass.Q - adduct = mass.Q
if(ion.mode %in% c('P','p')){
  adduct.list <- adduct.table[is.na(adduct.table[,2])==FALSE,1:2]
  mass.Q <- premass.Q + adduct.list[adduct.No,2]
}
if(ion.mode %in% c('N','n')){
  adduct.list <- adduct.table[is.na(adduct.table[,4])==FALSE,3:4]
  mass.Q <- premass.Q + adduct.list[adduct.No,2]
}
...
if(ion.mode %in% c('P','p')){ms2.Q <- rbind(c(1.007276,0),ms2.Q)}
if(ion.mode %in% c('N','n')){ms2.Q <- rbind(c(-1.007276,0),ms2.Q)}
...
# calculate the CSS scores with the stds(|premass diff| <= mass.diff) in the database
...
for(l in 1:length(database)){
  ...
  if(is.null(database[[l]]$PrecursorMZ)==FALSE){if(abs(database[[l]]$PrecursorMZ-premass.Q) > mass.diff) next}
  ...
  # Br
  if(a[9,2]<= 1){
    mass.L <- round(sum(a[,2]*a[,3]),4)
  } else if (a[9,2] %% 2 ==0){
    mass.L <- round(sum(a[c(1:8,10),2]*a[c(1:8,10),3])+a[9,2]*79.917314,4)
  } else {
    mass.L <- round(sum(a[c(1:8,10),2]*a[c(1:8,10),3])+a[9,2]%/%2*80.916291+(a[9,2]%/%2+1)*78.91834,4)
  }
  mass.list <- mass.L + bruniquemass - mass.Q
  if(min(abs(mass.list)) > mz.tol) next
  ...
  score.matrix[h,6] <- l
  if(db.name=='MoNA_MCSlibrary.msp'){score.matrix[h,7] <- smiles.db[l,2]}
  if(db.name!='MoNA_MCSlibrary.msp'){score.matrix[h,7] <- smiles.L}
...
for(p in 1:nrow(score.matrix)){
  ...
  for(m in 1:length(a.table)){a[a[,1]==names(a.table)[m],2] <- a.table[m]}
  if(a[9,2]<= 1){
    std.mass <- round(sum(a[,2]*a[,3]),4)
  } else if (a[9,2] %% 2 ==0){
    std.mass <- round(sum(a[c(1:8,10),2]*a[c(1:8,10),3])+a[9,2]*79.917314,4)
  } else {
    std.mass <- round(sum(a[c(1:8,10),2]*a[c(1:8,10),3])+a[9,2]%/%2*80.916291+(a[9,2]%/%2+1)*78.91834,4)
  }
  options(digits = 6)
  #if same mass, dont go through following biotrans
  if(abs(std.mass-mass.Q)<=mz.tol){
    output.individual <- cbind(score.matrix[p,],adduct.list[adduct.No,1],matrix(0,ncol=8,nrow = 1),abs(std.mass-mass.Q),score.matrix[p,5])
  ...
  result <- result[abs(result[,17]) < mz.tol,]
  ...
  output.individual <- cbind(score.matrix[p,],adduct.list[adduct.No,1],heavyatom,result[,c(4,6,7,11,13,14,16,17,18)])
  colnames(output.individual) <- colnames(output)
  output <- rbind(output, output.individual)
}
...
if(nrow(output)>=1){
  ...
  colnames(output)[21:22] <- c('modified matched ratio','MCS search score')
  output[,21] <- (output[,2]/topNo)/(0.5*log10(100*output[,3]/topNo))
  output[,22] <- 70*output[,21] + 5*output[,1] + 10*(1-abs(output[,19])/mz.tol) + 10*15/(output[,11]+15)
...
setwd(data.path)
write.csv(output.rank,file='output.csv',row.names = FALSE)
