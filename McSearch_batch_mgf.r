#########################################
# This script is to do the batch McSearch against MCS library (MGF input).
# Shipei Xing, Mar 12, 2020
# Copyright @ The University of British Columbia

#########################################
# Parameter setting part
data.path <-'E:/McSearch/R_script/batch_search'         # Data path for "batch_search" folder. Use "/" instead of "\".
db.name <- 'Fiehn HILIC_MCS library.msp'                # The MS/MS spectral database name ending with ".msp"
MGFfile.name <- 'input.mgf'                             # The name of input mgf file.
...
setwd(data.path)
#input mgf
data <- readMgfData(MGFfile.name)
mz <- mz(data)
int <- intensity(data)
premass <- precursorMz(data)
rt <- rtime(data)
...
for(i in 1:length(data)){
  ...
  premass.Q <- premass[i]
  ms2.Q <- as.data.frame(cbind(as.data.frame(mz[i]),as.data.frame(int[i])))
  rt.Q <- rt[i]
